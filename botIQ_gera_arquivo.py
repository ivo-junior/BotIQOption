            ...
            if qtd_martingales.strip() != '':
                for horario in sorted(catalogacao[par]):
                    ...
                    for i in range(int(qtd_martingales)):
                        ...
                        mg_time = str(datetime.strptime((datetime.now()).strftime('%Y-%m-%d ') + mg_time,
                        ...
                        if mg_time in catalogacao[par]:
                            ...
                            catalogacao[par][horario]['mg' + str(i + 1)]['vermelha'] += catalogacao[par][mg_time][
                                                                                            'verde'] + soma['vermelha']
                            catalogacao[par][horario]['mg' + str(i + 1)]['doji'] += catalogacao[par][mg_time]['verde'] + \
                                                                                    soma['doji']

                            catalogacao[par][horario]['mg' + str(i)]['%'] = round(100 * (
                                        catalogacao[par][horario]['mg' + str(i)][
                                            'verde' if catalogacao[par][horario]['dir'] == 'CALl' else 'vermelha'] / (
                                                    catalogacao[par][horario]['mg' + str(i)]['verde'] +
                                                    catalogacao[par][horario]['mg' + str(i)]['vermelha'] +
                                                    catalogacao[par][horario]['mg' + str(i)]['doji'])))
        ...
        for horario in sorted(catalogacao[par]):
            ...
            if ok == True:
                msg = Fore.YELLOW + par + Fore.RESET + ' - ' + horario + ' - ' + (
                    Fore.GREEN if catalogacao[par][horario]['dir'] == 'CALL' else Fore.RED) + catalogacao[par][horario][
                          'dir'] + Fore.RESET + ' - ' + str(
                    catalogacao[par][horario]['%']) + '% - ' + Back.GREEN + Fore.BLACK + str(
                    catalogacao[par][horario]['verde']) + Back.RED + str(
                ...
                if qtd_martingales.strip() != '':
                    for i in range(int(qtd_martingales)):
                        ...
                        if str(catalogacao[par][horario]['mg' + str(i)]['%']) != 'N/A':
                            msg += '..| MG ' + str(i) + ' - ' + str(
                                catalogacao[par][horario]['mg' + str(i)]['%']) + '% - ' + Back.GREEN + Fore.BLACK + str(
                                catalogacao[par][horario]['mg' + str(i)]['verde']) + Back.RED + str(
                                catalogacao[par][horario]['mg' + str(i)]['vermelha']) + Back.RESET + Fore.RESET + str(
                                catalogacao[par][horario]['mg' + str(i)]['doji'])
                ...
                open('sinais_' + (datetime.now()).strftime('%Y-%m-%d') + '_' + str(timeframe) + 'M.txt', 'a').write(
                    (datetime.now()).strftime('%y-%m-%d') + ' ' + horario + ',' + par + ',' + catalogacao[par][horario][
                        'dir'].strip() + ',' + str(timeframe) + '\n')
